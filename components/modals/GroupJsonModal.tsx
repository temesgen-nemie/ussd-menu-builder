...
export default function GroupJsonModal() {
  const { groupJsonModal, closeGroupJson, nodes } = useFlowStore();

  if (!groupJsonModal?.isOpen) return null;

  const groupName = nodes.find(n => n.id === groupJsonModal.groupId)?.data.name || "Subflow";
      ...
      <div className="relative w-[90vw] max-w-4xl max-h-[85vh] bg-white rounded-3xl shadow-2xl border border-indigo-100 flex flex-col overflow-hidden transform animate-in zoom-in-95 duration-300">
        ...
        <div className="flex-1 p-8 overflow-auto bg-gray-50/50 font-mono text-sm leading-relaxed">
          <div className="bg-white rounded-2xl border border-indigo-50 shadow-inner p-6">
             <pre className="text-gray-800 whitespace-pre-wrap select-all">
               {groupJsonModal.json}
             </pre>
          </div>
        </div>

        <div className="px-8 py-4 bg-indigo-50/30 border-t border-indigo-50 flex justify-end">
           <button 
             onClick={() => {
                navigator.clipboard.writeText(groupJsonModal.json);
                // Optionally add a toast here
             }}
             className="px-6 py-2 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl shadow-lg shadow-indigo-200 transition-all active:scale-95 flex items-center gap-2"
           >
              <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
