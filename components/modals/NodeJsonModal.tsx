...
export default function NodeJsonModal() {
  const { nodeJsonModal, closeNodeJson, nodes } = useFlowStore();
  ...
  const nodeName = node?.data.name || node?.type || "Node";
  ...
  return (
    <div className="fixed inset-0 z-[100000] flex items-center justify-center">
      ...
      />
      <div className="relative w-[90vw] max-w-2xl max-h-[80vh] bg-white rounded-3xl shadow-2xl border border-indigo-100 flex flex-col overflow-hidden transform animate-in zoom-in-95 duration-300">
        <div className="flex items-center justify-between px-8 py-5 border-b border-indigo-50 bg-indigo-50/30">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-lg font-black text-gray-800 tracking-tight">Node JSON View</div>
              <div className="text-xs font-bold text-indigo-500 uppercase tracking-widest">{nodeName}</div>
            </div>
          </div>
          <button
        ...
        <div className="flex-1 p-8 overflow-auto bg-gray-50/50 font-mono text-sm leading-relaxed">
          <div className="bg-white rounded-2xl border border-indigo-50 shadow-inner p-6">
            <pre className="text-gray-500 whitespace-pre-wrap outline-none">
              {nodeJsonModal.json}
            </pre>
          </div>
        </div>
        ...
        <div className="px-8 py-4 bg-indigo-50/30 border-t border-indigo-50 flex justify-end gap-3">
          ...
          <button
            onClick={() => {
              navigator.clipboard.writeText(nodeJsonModal.json);
