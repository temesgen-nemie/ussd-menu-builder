...
interface PromptNextNode {
  routes: Array<{ when: { eq: string[] }; gotoFlow: string }>;
  default: string;
}
...
export default function InspectorPanel() {
  ...
  // initialized via key-prop remounting to avoid useEffect synchronization
  ...
  const [activeTab, setActiveTab] = React.useState<"details" | "routes">(
    "details"
  );
  ...
  const handleReset = () => {
    if (node.type === "prompt") {
        ...
        // reset local state for textareas
        setApiBodyText("{}");
        setHeadersText("{}");
        setResponseMapText("{}");
                  ...
                  <div>
                    ...
                    <div className="space-y-3 max-h-[400px] overflow-y-auto pr-1">
                      {(() => {
                        ...
                        const nextNode = node.data.nextNode as
                          | PromptNextNode
                          | undefined;
                        const routes =
                          nextNode &&
                        ...
                        return routes.map((route, idx: number) => {
                  ...
                  <div className="border-t pt-4">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <button
                        onClick={() => {
                          const currentRoutes =
                            (node.data.routes as ActionRoute[]) || [];
                          updateNodeData(node.id, {
                            routes: [
                              ...currentRoutes,
                              {
                    ...
                    <div className="space-y-4">
                      {((node.data.routes as ActionRoute[]) || []).map(
                        (route, idx) => (
                          <div
                            key={route.id || idx}
                      ...
                      {(!node.data.routes ||
                        (node.data.routes as ActionRoute[]).length === 0) && (
                        <div className="text-center py-6 text-xs text-gray-400 bg-white rounded-lg border-2 border-dashed border-gray-100">
                          No conditional routes defined. <br /> Flow will always
                          proceed to the Default Path.
                        </div>
                      )}
                    </div>
                  </div>
